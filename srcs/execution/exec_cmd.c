...
/*   Updated: 2025/02/17 16:58:49 by jacobmaizel      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int exec_simple_cmd(t_pip *cmd, t_exec *exec)
{
    ...
    // Gestion des heredocs modifiée
    if (cmd->redirection && cmd->redirection->heredoc_count > 0)
    {
        if (handle_heredoc(cmd->redirection->heredoc_delim, 
                          cmd->redirection->heredoc_count) == -1)
            return (1);
